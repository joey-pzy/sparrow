...
public abstract class AbstractRowFilter implements Filter {
    ...
    public void filter(CanalEntry.Entry entry) {
        ...
        for (CanalEntry.RowData rowData : rowChange.getRowDatasList()) {
            if (eventType == CanalEntry.EventType.DELETE) {
                ...
                processInsert(rowData.getAfterColumnsList(), null);
            } else {
    ...
    protected abstract void processInsert(List<CanalEntry.Column> columns, String index);

    public void processDelete(List<CanalEntry.Column> columns) {
        log.debug("delete, data : {}", CanalColumnUtil.toColumnMap(columns));
    }

    protected abstract void processUpdate(List<CanalEntry.Column> beforeColumns, List<CanalEntry.Column> afterColumns);
}
